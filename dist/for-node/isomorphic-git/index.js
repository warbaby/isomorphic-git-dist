...
var locale2 = _interopDefault(require('locale2'));
var i18n = _interopDefault(require('simplest-i18n'));
...
async function writeObjectLoose ({ fs: _fs, gitdir, type, object, format, oid }) {
  const fs = new FileSystem(_fs);
  if (format !== 'deflated') {
    throw new GitError(E.InternalFail, {
...
async function writeObject ({ fs: _fs, gitdir, type, object, format = 'content', oid }) {
  const fs = new FileSystem(_fs);
  if (format !== 'deflated') {
    if (format !== 'wrapped') {
...
[name, value] = extractedVariable;
      ...
      const path$$1 = getPath(section, subsection, name);
      return { line, isSection, section, subsection, name, value, path: path$$1 }
    });
  }
  static from (text) {
  ...
  async get (path$$1, getall = false) {
    const allValues = this.parsedConfig
      .filter(config => config.path === path$$1.toLowerCase())
  ...
  async set (path$$1, value, append = false) {
    const configIndex = findLastIndex(
      this.parsedConfig,
      config => config.path === path$$1.toLowerCase()
    );
    if (value == null) {
      if (configIndex !== -1) {
...
async function listpack (stream$$1, onData) {
  let reader = new StreamReader(stream$$1);
  let hash = new Hash();
...
function readPack (fs, filename, getExternalRefDelta) {
...
async function readObjectPacked ({ fs: _fs, gitdir, oid, format = 'content', getExternalRefDelta }) {
  const fs = new FileSystem(_fs);
  ...
  for (let entry of tree) {
    ...
    switch (type) {
      case 'blob':
        ...
        } else if (entry.mode === '160000') ; else {
          throw new GitError(E.InternalFail, {
...
class GitRemoteHTTP {
  ...
  static async connect ({ corsProxy, service, url, noGitSuffix, auth, stream: stream$$1 }) {
    // Auto-append the (necessary) .git if it's missing.
    if (!url.endsWith('.git') && !noGitSuffix) url = url += '.git';
    if (corsProxy) {
      url = `${corsProxy}/${url.replace(/^https?:\/\//, '')}`;
...
async function expandOidPacked ({ fs: _fs, gitdir, oid: short, getExternalRefDelta }) {
  const fs = new FileSystem(_fs);
  // Iterate through all the .pack files
  let results = [];
  let list = await fs.readdir(path.join(gitdir, '/objects/pack'));
...
async function resetIndex ({
  ...
  try {
    ...
    try {
      // Resolve blob
      const obj = await readObject$1({ gitdir, fs, oid, filepath, format: 'deflated' });
      oid = obj && obj.oid;
    } catch (e) {
...
const path$1 = (path.posix === undefined) ? path : path.posix;
...
class GitWalkerFs {
  ...
  async readdir (entry) {
    ...
    let { fs, dir } = this;
    let names = await fs.readdir(path$1.join(dir, filepath));
    if (names === null) return null
    return names.map(name => ({
      fullpath: path$1.join(filepath, name),
      basename: name,
      exists: true
    }))
  }
...
class GitWalkerRepo {
  ...
  async readdir (entry) {
    ...
    for (const entry of tree) {
      map.set(path$1.join(filepath, entry.path), entry);
    }
    return tree.entries().map(entry => ({
      fullpath: path$1.join(filepath, entry.path),
      basename: entry.path,
      exists: true
    }))
  }
